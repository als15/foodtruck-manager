    ...
    <Box>
                ...
                {expenseCategories.map((category) => (
                  <Box key={category.id} sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                    ...
                    <Chip 
                      label={category.type} 
                      size="small" 
                      color={category.type === 'fixed' ? 'primary' : category.type === 'variable' ? 'warning' : 'default'}
                    />
                  </Box>
                ))}
              </CardContent>
                          ...
                          <TableRow key={expense.id}>
                            ...
                            <TableCell>
                              <Chip 
                                label={expense.type} 
                                size="small"
                                color={expense.type === 'fixed' ? 'primary' : expense.type === 'variable' ? 'warning' : 'default'}
                              />
                            </TableCell>
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12} md={6}>
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                      ...
                      {analysis.usingCalculatedValues.orderValue && (
                        <Chip label="Calculated" size="small" sx={{ ml: 1 }} />
                      )}
                    </Typography>
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                      Average Profit Margin: <strong>{analysis.avgProfitMargin.toFixed(1)}%</strong>
                      {analysis.usingCalculatedValues.profitMargin && (
                        <Chip label="Calculated" size="small" sx={{ ml: 1 }} />
                      )}
                    </Typography>
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                  ...
                  <Grid item xs={12} md={6}>
                      ...
                      <Grid container spacing={1}>
                        <Grid item xs={4}>
                          <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'warning.light', borderRadius: 1 }}>
                            <Typography variant="caption" sx={{ fontWeight: 'bold' }}>THU</Typography>
                        ...
                        <Grid item xs={4}>
                          <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'success.light', borderRadius: 1 }}>
                            <Typography variant="caption" sx={{ fontWeight: 'bold' }}>FRI</Typography>
                        ...
                        <Grid item xs={4}>
                          <Box sx={{ textAlign: 'center', p: 1, bgcolor: 'info.light', borderRadius: 1 }}>
                            <Typography variant="caption" sx={{ fontWeight: 'bold' }}>SAT</Typography>
                    ...
                    <Alert severity={menuItems.length > 0 ? "success" : "warning"}>
                      {menuItems.length > 0 
                        ? `Using data from ${menuItems.filter(i => i.isAvailable).length} menu items • 3-day schedule`
                        : 'Add menu items for more accurate calculations'
                      }
                    </Alert>
                  </Grid>
                </Grid>
                ...
                <CardContent>
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={6}>
                      ...
                      <Typography 
                        variant="h6" 
                        color={projection.projectedProfit >= 0 ? "success.main" : "error.main"}
                      >
                    ...
                    <Grid item xs={6}>
                      ...
                      <Typography variant="h6">
                        {projection.profitMarginPercentage.toFixed(1)}%
                      </Typography>
                    </Grid>
                    <Grid item xs={12}>
            ...
            <Grid item xs={12} md={6} key={goal.id}>
              <Card>
                <CardContent>
                  ...
                  <Chip 
                    label={goal.type.replace('_', ' ').toUpperCase()} 
                    size="small" 
                    color={goal.isActive ? 'primary' : 'default'}
                  />
                </CardContent>
              </Card>
            </Grid>
      ...
      <TabPanel value={currentTab} index={4}>
        ...
        <Alert severity="info" sx={{ mb: 3 }}>
          Reports and visualizations will be implemented in the next phase. 
          This will include charts, trends, and detailed financial analytics.
        </Alert>
      </TabPanel>
      ...
      <Dialog open={openGoalDialog} onClose={() => setOpenGoalDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button onClick={handleSaveGoal} variant="contained">Create Goal</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={openProjectionDialog} onClose={() => setOpenProjectionDialog(false)} maxWidth="md" fullWidth>
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                type="number"
                inputProps={{ step: "0.01" }}
                value={newProjection.averageOrderValue}
                onChange={(e) => setNewProjection({ ...newProjection, averageOrderValue: parseFloat(e.target.value) || 0 })}
                helperText="Average amount per customer order"
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                type="number"
                value={newProjection.ordersPerDay}
                onChange={(e) => setNewProjection({ ...newProjection, ordersPerDay: parseInt(e.target.value) || 0 })}
                helperText="Expected number of orders per day"
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                type="number"
                value={newProjection.workingDaysPerMonth}
                onChange={(e) => setNewProjection({ ...newProjection, workingDaysPerMonth: parseInt(e.target.value) || 22 })}
                helperText="Number of operating days per month"
              />
            </Grid>
            <Grid item xs={12}>
              <Alert severity="info">
                Based on your current monthly expenses of ${calculateMonthlyExpenses().toFixed(2)}, 
                this projection will calculate your expected profit and break-even point.
              </Alert>
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
          ...
          <Button onClick={handleCalculateProjection} variant="contained">Calculate Projection</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={openSettingsDialog} onClose={() => setOpenSettingsDialog(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={3} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              <Typography variant="h6" sx={{ mb: 2 }}>Order Value & Profit Margin</Typography>
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField
                fullWidth
                ...
                helperText={financialSettings.customAverageOrderValue === 0 
                  ? `Using calculated: $${calculateAverageOrderValue().toFixed(2)}`
                  : "Leave 0 to use calculated value from menu items"
                }
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                helperText={financialSettings.customProfitMargin === 0 
                  ? `Using calculated: ${(calculateAverageProfitMargin() * 100).toFixed(1)}%`
                  : "Leave 0 to use calculated value from menu items"
                }
              />
            </Grid>
            ...
            <Grid item xs={12}>
              <Typography variant="h6" sx={{ mb: 2, mt: 2 }}>3-Day Operating Schedule</Typography>
            ...
            <Grid item xs={12}>
              <Typography variant="h6" sx={{ mb: 2, mt: 2 }}>Daily Performance Multipliers</Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            ...
            <Grid item xs={12}>
              <Typography variant="h6" sx={{ mb: 2, mt: 2 }}>Seasonal Adjustments</Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            ...
            <Grid item xs={12}>
              <Alert severity="info" sx={{ mt: 2 }}>
                <Typography variant="body2">
                  <strong>Current Season:</strong> {getCurrentSeasonMultiplier()}x multiplier is being applied to break-even calculations.
                  Values above 1.0 indicate higher demand (easier to reach break-even), below 1.0 indicate lower demand.
                </Typography>
              </Alert>
            </Grid>
          </Grid>
