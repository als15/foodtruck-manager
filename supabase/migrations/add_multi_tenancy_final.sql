...
CREATE OR REPLACE FUNCTION add_business_id_to_table(target_table TEXT)
RETURNS VOID AS $$
BEGIN
  ...
  IF EXISTS (
    ...
    IF NOT EXISTS (
      ...
      EXECUTE format('ALTER TABLE %I ADD COLUMN business_id UUID REFERENCES businesses(id) ON DELETE CASCADE', target_table);
    END IF;
  END IF;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION create_business_id_index(target_table TEXT)
RETURNS VOID AS $$
BEGIN
  IF EXISTS (
    SELECT FROM information_schema.tables 
    ...
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_business_id ON %I(business_id)', target_table, target_table);
  END IF;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  RETURN EXISTS (
    ...
    AND user_businesses.business_id = check_business_id
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Create function to get user's role in business
CREATE OR REPLACE FUNCTION get_user_business_role(check_business_id UUID)
RETURNS TEXT AS $$
BEGIN
  RETURN (
...
CREATE OR REPLACE FUNCTION create_business_rls_policies(target_table TEXT)
RETURNS VOID AS $$
BEGIN
  IF EXISTS (
    SELECT FROM information_schema.tables 
    ...
    -- Create policies
    EXECUTE format('CREATE POLICY "Users can only see data from their businesses" ON %I FOR SELECT USING (user_has_business_access(business_id))', target_table);
    EXECUTE format('CREATE POLICY "Users can insert data to their businesses" ON %I FOR INSERT WITH CHECK (user_has_business_access(business_id))', target_table);
    EXECUTE format('CREATE POLICY "Users can update data in their businesses" ON %I FOR UPDATE USING (user_has_business_access(business_id))', target_table);
    EXECUTE format('CREATE POLICY "Users can delete data from their businesses" ON %I FOR DELETE USING (user_has_business_access(business_id))', target_table);
  END IF;
EXCEPTION
